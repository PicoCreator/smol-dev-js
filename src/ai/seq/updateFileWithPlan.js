...
module.exports = async function updateFileWithPlan(fileType, filePath, plan, depStr, commonCtx) {
	...
	if( fileType === "spec" ) {
		promptArr.push([
			...
			`   - DO NOT provide polite greating before the markdown file content, or any other text, only the markdown content`,
			`	- MOST IMPORTANT OF ALL: every line of markdown you generate must be valid markdown code. Do not include markdown code fences in your response`,
			"",
		...
		promptArr.push([
			...
			`	- do not stray from the plan, or the names of the files and the dependencies we have shown above`,
			`	- MOST IMPORTANT OF ALL: every line of code you generate must be valid code. Do not include markdown code fences in your response. Do not add file headers in your response`,
			"",
